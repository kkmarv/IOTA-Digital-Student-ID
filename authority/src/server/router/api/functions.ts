...
import {
  RegistrationData,
  StudyData,
} from "../../../identity/subjects/Matriculation.js";
...
const TANGLE_RESOLVER = await Resolver.builder()
...
export function sendChallenge(req: Request, res: Response, next: NextFunction) {
  ...
  try {
    ...
    console.error(err);
    return next(
      new HTTPError(HTTPCode.UNPROCESSABLE_ENTITY, "Not a valid DID.")
    );
  }
...
export async function register(
  ...
  if (registrationData === null) {
    return next(
      new HTTPError(HTTPCode.UNPROCESSABLE_ENTITY, "Bad registration data.")
    );
  }
  ...
  if (!CHALLENGE_MAP.has(registrationData.id.toString())) {
    return next(
      new HTTPError(HTTPCode.UNAUTHORIZED, "Please get a challenge first.")
    );
  }
...
export async function login(req: Request, res: Response, next: NextFunction) {
  ...
  try {
    ...
    assert(
      studentVP.verifiableCredential() !== undefined,
      "Credential property missing."
    );
  } catch (err) {
    console.error(err);
    if (err instanceof Error) {
      return next(
        new HTTPError(
          ...
          "Not a verifiable presentation:\n" + err.message
        )
      );
    } else {
      return next(
        new HTTPError(HTTPCode.BAD_REQUEST, "Not a verifiable presentation.")
      );
    }
  }
  ...
  try {
    ...
    return next(
      new HTTPError(
        ...
        "Presentation contains invalid DID."
      )
    );
  }
