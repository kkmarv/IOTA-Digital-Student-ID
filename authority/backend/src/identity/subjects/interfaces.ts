...
export function isIStudentData(data: any): data is IStudentData {
  return (
    ...
    data.birthDate &&
    ((typeof data.birthDate === 'string' && !isNaN(Date.parse(data.birthDate))) || data.birthDate instanceof Date) &&
    data.address &&
...
export function isIStudySubject(data: any): data is IStudySubject {
  return data && data.name && typeof data.name === 'string' && data.degree && isUniversityDegree(data.degree)
}
